...
public class DWAOAuthService {
  ...
  public Token getRequestToken() {
    ...
    // requestPostMethod.addRequestHeader(OAuthConstants.HEADER,requestTokenRequest.getHeaders().get(OAuthConstants.HEADER));

    // HttpClient httpClient = createSession();
    ...
    try {
      ...
      System.out.println("Request Token" + requestPostMethod.getResponseBodyAsString());
  ...
  public void signRequest(final Token token, final HttpMethodBase requestMethod, final boolean isTokenRequired) {
    ...
    // // request.addOAuthParameter("realm", "DWA");
  ...
  public Token getAccessToken(final Token requestToken) {
    ...
    // accessTokenPostMethod.addRequestHeader(OAuthConstants.HEADER,accessTokenRequest.getHeaders().get(OAuthConstants.HEADER));
    // HttpClient createSession = createSession();

    int accessTokenRequestResponse = -1;

    try {
      accessTokenRequestResponse = this.httpClient.executeMethod(accessTokenPostMethod);
      System.out.println(accessTokenRequestResponse);
    }
    catch (HttpException e1) {
      e1.printStackTrace();
    }
    ...
    try {
      ...
      System.out.println(accessTokenRequestResponse);
      System.out.println("Access Token" + accessTokenPostMethod.getResponseBodyAsString());
  ...
  public void doAuthentication(final String authorizationURL, final Token token, final String username,
    ...
    createSession();
    // OAuthRequest request_access2 = new OAuthRequest(Verb.GET, authorizationUrl);
    // service.signRequest(requestToken, request_access2);
    // Response send2 = request_access2.send();

    GetMethod authorizeGetMethod = new GetMethod(authorizationURL);
    // authorizeGetMethod.addRequestHeader(OAuthConstants.HEADER ,
    // request_access2.getHeaders().get(OAuthConstants.HEADER));
    ...
    try {
      System.out.println(authorizeGetMethod.getResponseBodyAsString());
    }
    catch (IOException e) {
      e.printStackTrace();
    }
    ...
    // try {
    // System.out.println(authenticateMethod.getResponseBodyAsString());
    // } catch (IOException e) {
    // e.printStackTrace();
    // }
  ...
  public String getResponse(final Token accessToken, final String path, final String queryString) {
    // String encodePathQuery = path;
    // try {
    // if ((queryString != null) && !queryString.isEmpty()) {
    // encodePathQuery = URIUtil.encodePathQuery(path + queryString);
    // }
    //
    // }
