...
public abstract class TransitionHelperAdapter implements ITransitionHelper {
   ...
   public boolean isSystemUser() throws OseeCoreException {
      return AtsUserService.get().isGuestUser(AtsUsersClient.getUser()) || AtsUserService.get().isSystemUser(AtsUsersClient.getUser());
   }

   @Override
   public boolean isSystemUserAssingee(AbstractWorkflowArtifact awa) throws OseeCoreException {
      return awa.getStateMgr().getAssignees().contains(AtsUserService.get().getGuestUser()) || awa.getStateMgr().getAssignees().contains(
