...
public class ServletRegistrationHandler {
   ...
   public void setCachingService(TempCachingService caching) {
   ...
   private void registerServices(BundleContext context) {
      ...
      register(
         new OseeCacheServlet(logger, sessionManager, translationService, branchCache, caching.getTransactionCache(),
            orcsApi, factoryService, identityService), OseeServerContext.CACHE_CONTEXT);
      register(new OseeModelServlet(logger, sessionManager, translationService, orcsApi, identityService),
         OseeServerContext.OSEE_MODEL_CONTEXT);
      register(new UnsubscribeServlet(logger, context, orcsApi), "osee/unsubscribe");

      register(new AtsServlet(logger, resourceManager, branchCache), "osee/ats");
      register(new ConfigurationServlet(logger, translationService, orcsApi), OseeServerContext.OSEE_CONFIGURE_CONTEXT);
      register(new DataServlet(logger, resourceManager, branchCache), "osee/data");
