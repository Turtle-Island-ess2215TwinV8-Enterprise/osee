...
public abstract class SearchableCollection<MATCH_DATA, KEY, DATA> {
   ...
   protected List<DATA> getListByFilter(DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      return getListByFilter(getAll(), matcher);
   }

   protected ResultSet<DATA> getResultSetByFilter(DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      return getResultSetByFilter(getAll(), matcher);
   }

   protected <T extends DATA> ResultSet<T> getSetByFilter(KEY type, DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      List<T> result = getListByFilter(type, matcher);
      ResultSet<T> resultSet = createResultSet(type, result);
      return resultSet;
   }

   protected <T extends DATA> List<T> getListByFilter(KEY type, DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      return getListByFilter(getAllByType(type), matcher);
   }

   protected boolean hasItemMatchingFilter(DataMatcher<MATCH_DATA> matcher) {
      return hasItemMatchingFilter(getAll(), matcher);
   }

   protected boolean hasItemMatchingFilter(KEY type, DataMatcher<MATCH_DATA> matcher) {
      return hasItemMatchingFilter(getAllByType(type), matcher);
   }

   private ResultSet<DATA> getResultSetByFilter(Collection<DATA> source, DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      List<DATA> values = getListByFilter(source, matcher);
      return createResultSet(values);
   }

   @SuppressWarnings("unchecked")
   private <T extends DATA> List<T> getListByFilter(Collection<DATA> source, DataMatcher<MATCH_DATA> matcher) throws OseeCoreException {
      List<T> toReturn;
   ...
   private boolean hasItemMatchingFilter(Collection<DATA> source, DataMatcher<MATCH_DATA> matcher) {
